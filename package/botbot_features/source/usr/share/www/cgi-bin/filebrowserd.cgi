...
function doneSelect(){
	if ( folderchoosen.efolder.value == 'yes'){
		...
		var selfolder = folderlist.folder.value;
		if (selfolder == ''){
			alert("Error: You have to select a folder to share.");
			return false;
			}
		else {
			folderchoosen.shared.value = selfolder;
			return true;
			}
		}
}
...
<b>Current folder: <%= ${CURDIR:-none} %></b><br>
